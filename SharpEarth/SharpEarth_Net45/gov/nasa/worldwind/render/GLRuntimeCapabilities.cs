...
public class GLRuntimeCapabilities
{
     ...
     * <p/>
     * Note: The default vertex-buffer usage flag can be set via {@link SharpEarth.Configuration} using the key
     ...
     *
     * @throws ArgumentException if the glContext is null.
     */
    public void initialize(GLContext glContext)
    {
        ...
        if (this.depthBits == 0)
        {
            ...
            gl.glGetIntegerv(GL.GL_DEPTH_BITS, parameters, 0);
            this.depthBits = parameters[0];
        }
        ...
        if (this.maxTextureAnisotropy < 0)
        {
            ...
            if (this.isAnisotropicTextureFilterAvailable)
            {
                ...
                gl.glGetFloatv(GL.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, parameters, 0);
                this.maxTextureAnisotropy = parameters[0];
            }
        }
        ...
        {
            int[] parameters = new int[1];
            gl.glGetIntegerv(GL2.GL_MAX_TEXTURE_UNITS, parameters, 0);
            this.numTextureUnits = parameters[0];
        }

        if (this.maxTextureSize == 0)
        {
            int[] parameters = new int[1];
            gl.glGetIntegerv(GL.GL_MAX_TEXTURE_SIZE, parameters, 0);
            this.maxTextureSize = parameters[0];
        }
    }
     ...
     *
     * @throws ArgumentException if depthBits is less than one.
     */
     ...
     *
     * @throws ArgumentException if the size is less than one.
     */
     ...
     *
     * @throws ArgumentException if the number of texture units is less than one.
     */
