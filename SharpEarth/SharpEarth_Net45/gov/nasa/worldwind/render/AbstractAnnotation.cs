 ...
 * ...
 * </pre>
 * <p/> Annotations are usually handled by an {@link SharpEarth.layers.AnnotationLayer}. Although they also
 * implement the {@link Renderable} interface and thus can be handled by a {@link SharpEarth.layers.RenderableLayer}
 * too. <p/>
 * <pre>
 ...
 * <p/>
 * Background images are specified by setting the Annotation attribute {@link SharpEarth.render.AnnotationAttributes#setImageSource(Object)}.
 ...
 * background image may be translated or scaled by setting the attributes {@link SharpEarth.render.AnnotationAttributes#setImageOffset(java.awt.Point)}
 * and {@link SharpEarth.render.AnnotationAttributes#setImageScale(double)}, respectively. The offset attribute
 ...
 * the original image. The utility method {@link SharpEarth.util.WWMath#powerOfTwoCeiling(int)} is useful for
...
public abstract class AbstractAnnotation : AVListImpl , Annotation
{
    ...
    protected double minActiveAltitude = -Double.MaxValue;
    protected double maxActiveAltitude = Double.MaxValue;
...
//        mltr.setLineSpacing(-2);
     ...
     * ArgumentException. Unknown structures in <code>stateInXml</code> are benign, because they will simply be
     * ignored.
     *
     * @param stateInXml an XML document String describing an AbstractAnnotation.
     *
     * @throws ArgumentException If <code>stateInXml</code> is null, or if <code>stateInXml</code> is not a well
