...
/**
 * Builds a list of {@link SharpEarth.render.SurfaceTile} instances who's content is defined by a specified set
 * of {@link SharpEarth.render.SurfaceRenderable} instances. It's typically not necessary to use
 ...
 * use or extend {@link SharpEarth.render.SurfaceShape} instead of using SurfaceObjectTileBuilder directly.
 * <p/>
 ...
 * during the preRender method of a World Wind layer. See the {@link SharpEarth.render.PreRenderable} interface
 * for details. Once built, the surface tiles can be rendered by a {@link SharpEarth.render.SurfaceTileRenderer}.
 * <p/>
...
public class SurfaceObjectTileBuilder
{
     ...
     *
     * @throws ArgumentException if the dimension is null.
     */
     ...
     * SharpEarth.View}. This may temporarily use the framebuffer to perform offscreen rendering, and therefore
     * should be called during the preRender method of a World Wind {@link SharpEarth.layers.Layer}.
     * <p/>
    ...
    protected static LevelSet createLevelSet(int tileWidth, int tileHeight)
    {
        ...
        parameters.setValue(AVKey.LEVEL_ZERO_TILE_DELTA, DEFAULT_LEVEL_ZERO_TILE_DELTA);
        parameters.setValue(AVKey.SECTOR, Sector.FULL_SPHERE);
        parameters.setValue(AVKey.NUM_LEVELS, DEFAULT_NUM_LEVELS);
        parameters.setValue(AVKey.NUM_EMPTY_LEVELS, 0);
        parameters.setValue(AVKey.TILE_WIDTH, tileWidth);
        parameters.setValue(AVKey.TILE_HEIGHT, tileHeight);
        // This is a shared LevelSet, so just supply a dummy cache name and dataset name.
        parameters.setValue(AVKey.DATA_CACHE_NAME, SurfaceObjectTileBuilder.class.getName());
        parameters.setValue(AVKey.DATASET_NAME, SurfaceObjectTileBuilder.class.getName());
        // We won't use any tile resource paths, so just supply a dummy format suffix.
        parameters.setValue(AVKey.FORMAT_SUFFIX, SurfaceObjectTileBuilder.class.getName());
     ...
     * SharpEarth.geom.Sector)}. Tiles that don't intersect any surface renderables are discarded.
     *
     ...
     * See {@link #meetsRenderCriteria(DrawContext, SharpEarth.util.LevelSet, SharpEarth.util.Tile)}.
     *
     ...
     * the {@link SharpEarth.View} attached to the draw context.
     *
     * @param dc   the current draw context.
     * @param tile the tile to test.
     *
    ...
    /**
     * Represents a {@link SharpEarth.layers.TextureTile} who's contents is constructed by a set of surface
    ...
    protected static class SurfaceObjectTile extends TextureTile
    {
         ...
         *
         * @throws ArgumentException if any of the {@code sector}, {@code level}, or {@code cacheName } are
         *                                  {@code null}.
         */
