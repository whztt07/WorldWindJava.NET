...
using SharpEarth.formats.dds.DDSCompressor;
...
//        }
     ...
     * @throws ArgumentException if any of the required parameters or values are missing
     */
    public DataRaster composeRaster(AVList reqParams) throws ArgumentException, WWRuntimeException
    {
     ...
     *
     * @param parameters This is a required parameter, must not be null or empty; Must contain AVKey.WIDTH, AVKey.HEIGHT,
     ...
     * @throws ArgumentException if any of the required parameters or values are missing
     */
    public ByteBuffer getRasterAsByteBuffer(AVList parameters)
    {
        // request may contain a specific file format, different from a default file format
        String format = (null != parameters && parameters.hasKey(AVKey.IMAGE_FORMAT))
            ? parameters.getStringValue(AVKey.IMAGE_FORMAT) : this.getStringValue(AVKey.IMAGE_FORMAT);
