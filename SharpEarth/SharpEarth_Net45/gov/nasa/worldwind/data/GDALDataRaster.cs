...
using org.gdal.gdalconst.gdalconst;
using org.gdal.gdal;
...
public class GDALDataRaster : AbstractDataRaster , Cacheable
{
     ...
     */
    public GDALDataRaster(Object source) throws ArgumentException, FileNotFoundException
    {
        this(source, false);
    }
     ...
     *
     * @throws ArgumentException if the Sector is null
     */
    public void setSector(Sector sector) throws ArgumentException
    {
        if (null == sector)
        {
...
//                throw new WWRuntimeException( message );
    ...
    protected void init(Dataset ds, bool quickReadingMode)
    {
        ...
        GDALMetadata.extractExtendedAndFormatSpecificMetadata(ds, extParams, parameters);
    ...
    protected void doDrawOnTo(DataRaster canvas)
    {
        try
        {
            ...
            DataRaster raster = this.doGetSubRaster(clipRect.width, clipRect.height, overlap, parameters );
...
//        Double nodata = this.calcNoDataForDestinationRaster(destParams);
    ...
    public DataRaster getSubRaster(AVList parameters)
    {
        ...
        {
            GDALUtils.extractBandOrder(this.dsVRT, parameters);
        }
