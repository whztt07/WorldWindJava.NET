...
using java.awt;
using javax.swing;
using javax.media.opengl.awt.AWTGLAutoDrawable;
using javax.media.opengl;
using SharpEarth.util.dashboard.DashboardController;
using SharpEarth.util;
using SharpEarth.render.ScreenCreditController;
using SharpEarth.pick;
using SharpEarth.geom.Position;
using SharpEarth.exception.WWAbsentRequirementException;
using SharpEarth.events;
using SharpEarth.cache.GpuResourceCache;
using com.jogamp.opengl.util.texture.TextureIO;
...
public class WorldWindowGLAutoDrawable : WorldWindowImpl , WorldWindowGLDrawable, GLEventListener
{
     ...
     *
     * @see SharpEarth.View#getViewStateID()
     */
...
//        this.drawable.setGL(new DebugGL(this.drawable.getGL())); // uncomment to use the debug drawable
     ...
     * of life, its resources should be released by calling {@link SharpEarth.WorldWindow#shutdown()} or {@link
     * SharpEarth.WorldWind#shutDown()}, respectively. In the case when a WorldWindow is removed from its parent
