...
using SharpEarth.util.xml.xal.XALAddressDetails;
using SharpEarth.util.xml.atom;
 ...
 * SharpEarth.render.DrawContext)}</code> and <code>{@link #doRender(gov.nasa.worldwind.ogc.kml.impl.KMLTraversalContext,
 * SharpEarth.render.DrawContext)}</code> to render their contents. If the <code>visibility</code> property is
...
public abstract class KMLAbstractFeature extends KMLAbstractObject implements KMLRenderable
{
    ...
    /**
     * Draws the <code>{@link SharpEarth.render.Balloon}</code> associated with this KML feature. This does
     ...
     * sub-style is marked with the value {@link SharpEarth.avlist.AVKey#UNRESOLVED}. The same is true when a
     ...
     * @param subStyle   an instance of the sub-style desired, such as {@link SharpEarth.ogc.kml.KMLIconStyle}.
