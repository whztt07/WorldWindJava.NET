...
using java.util.concurrent.CopyOnWriteArraySet;
using java.awt.color;
using java.awt;
using org.gdal.osr;
using org.gdal.ogr.ogr;
...
//                    gdalNativeLibraryLoaded = gdalLoadNativeLibrary(true);
     ...
     * @param parameters AVList of parameters
     *
     ...
     * @throws WWRuntimeException       if GDAL library was not initialized
     */
    protected static DataRaster composeImageDataRaster(Dataset ds, AVList parameters)
        throws ArgumentException, SecurityException, WWRuntimeException
    {
        if (!gdalIsAvailable.get())
        {
            String message = Logging.getMessage("gdal.GDALNotAvailable");
        ...
        for (int bandIdx = 0; bandIdx < bandCount; bandIdx++)
        {
            ...
            {
                maxValue = (Double) parameters.getValue(AVKey.RASTER_BAND_MAX_PIXEL_VALUE);
            }
        ...
        {
            actualBitsPerColor = (Integer) parameters.getValue(AVKey.RASTER_BAND_ACTUAL_BITS_PER_PIXEL);
        }
        else if (maxValue > 0d)
        {
        ...
        try
        {
            reqBandOrder = extractBandOrder(ds, parameters);
        ...
        return BufferedImageRaster.wrap(img, parameters);
    }
...
//        BufferedImage flipImg = new BufferedImage(w, h, img.getColorModel().getTransparency() );
    ...
    protected static IntBuffer extractImageMask(AVList parameters)
    {
        if (null == parameters || !params.hasKey(AVKey.GDAL_MASK_DATASET))
        {
            return null;
        }
     ...
     *
     * @throws ArgumentException if sector is null, or raster size is zero
     */
    public static double[] calcGetGeoTransform(Sector sector, int width, int height) throws ArgumentException
    {
        if (null == sector)
        {
...
//      GeoTransform[3] -= 0.5 * GeoTransform[5];
    ...
    public static AVList extractRasterParameters(Dataset ds) throws ArgumentException, WWRuntimeException
    {
     ...
     * @param parameters           AVList to hold retrieved metadata, if null, a new instance will be created and returned
     *                         as a return value
     ...
     *
     * @throws ArgumentException when the passed dataset is null pr emtpy, or any of the dimension is 0
        ...
        throws ArgumentException, WWRuntimeException
    {
        ...
        parameters.setValue(AVKey.NUM_BANDS, bandCount);
        ...
        if (null != band)
        {
            ...
            {
                parameters.setValue(AVKey.RASTER_HAS_OVERVIEWS, Boolean.TRUE);
            }
            ...
            {
                parameters.setValue(AVKey.PIXEL_FORMAT, AVKey.ELEVATION);
                parameters.setValue(AVKey.DATA_TYPE, AVKey.INT32);
            }
            else if (dataType == gdalconst.GDT_Float32 || dataType == gdalconst.GDT_CFloat32)
            {
                parameters.setValue(AVKey.PIXEL_FORMAT, AVKey.ELEVATION);
                parameters.setValue(AVKey.DATA_TYPE, AVKey.FLOAT32);
            }
            else if (dataType == gdalconst.GDT_Byte)
            {
                ...
                if (colorInt == gdalconst.GCI_GrayIndex && bandCount < 3) {
                    parameters.setValue(AVKey.IMAGE_COLOR_FORMAT, AVKey.GRAYSCALE);
                } else {
                    // if has only one band => one byte index of the palette, 216 marks voids
                    parameters.setValue(AVKey.IMAGE_COLOR_FORMAT, AVKey.COLOR);
                }
                parameters.setValue(AVKey.PIXEL_FORMAT, AVKey.IMAGE);
                parameters.setValue(AVKey.DATA_TYPE, AVKey.INT8);
            }
            else if (dataType == gdalconst.GDT_UInt16)
            {
                parameters.setValue(AVKey.IMAGE_COLOR_FORMAT,
                ...
                && !params.hasKey(AVKey.ELEVATION_UNIT) )
            {
                ...
                try
                {
                    ...
                    WWUtil.copyValues( tiffParams, parameters, new String[] { AVKey.ELEVATION_UNIT,
              ...
              )
            {
                ...
                {
                    parameters.setValue(AVKey.MISSING_DATA_SIGNAL, minmax[0]);

                    if( setNoDataValue( band, minmax[0]) )
                    {
                        band.ComputeRasterMinMax(minmax);

                        parameters.setValue(AVKey.ELEVATION_MIN, minmax[0]);
                        parameters.setValue(AVKey.ELEVATION_MAX, minmax[1]);
                    }
                }
                else
                {
        ...
        {
            proj_wkt = parameters.getStringValue(AVKey.SPATIAL_REFERENCE_WKT);
        }
        ...
        if (!WWUtil.isEmpty(proj_wkt))
        {
            parameters.setValue(AVKey.SPATIAL_REFERENCE_WKT, proj_wkt);
        ...
        {
            parameters.setValue(AVKey.COORDINATE_SYSTEM, AVKey.COORDINATE_SYSTEM_SCREEN);
        }
        ...
        if (null != srs)
        {
            ...
            if (!params.hasKey(AVKey.SPATIAL_REFERENCE_WKT))
            {
                parameters.setValue(AVKey.SPATIAL_REFERENCE_WKT, srs.ExportToWkt());
            }

            if (srs.IsLocal() == 1)
            {
                parameters.setValue(AVKey.COORDINATE_SYSTEM, AVKey.COORDINATE_SYSTEM_UNKNOWN);
...
//                throw new WWRuntimeException(msg);
            ...
            if (null != area)
            {
                parameters.setValue(AVKey.GDAL_AREA, area);
            ...
            {
                parameters.setValue(AVKey.COORDINATE_SYSTEM, AVKey.COORDINATE_SYSTEM_PROJECTED);
...
//                String projcs_unit = srs.GetAttrValue("PROJCS|GEOGCS|UNIT");
                ...
                {
                    parameters.setValue(AVKey.PROJECTION_NAME, projection);
                }
            }
...
//            String type = band.GetUnitType();
            ...
            if( null != minValue )
                parameters.setValue(AVKey.ELEVATION_MIN, minValue);

            if( null != maxValue )
                parameters.setValue(AVKey.ELEVATION_MAX, maxValue);

            if( null != missingSignal )
                parameters.setValue(AVKey.MISSING_DATA_SIGNAL, missingSignal );
        }
    }
    ...
    public static DataRaster composeDataRaster(Dataset ds, AVList parameters)
        throws ArgumentException, WWRuntimeException
    {
        if (!gdalIsAvailable.get())
        {
    ...
    public static int[] extractBandOrder(Dataset ds, AVList parameters)
        throws ArgumentException, WWRuntimeException
    {
        if (!gdalIsAvailable.get())
        {
     ...
     * @param parameters , The AVList with properties (usually used to force projection info or sector)
     *
     * @return ByteBufferRaster as DataRaster
     *
     * @throws ArgumentException if raster parameters (height, width, sector, etc) are invalid
     * @throws WWRuntimeException       when invalid raster detected (like attempt to use the method for imagery
     *                                  raster)
     */
    protected static DataRaster composeNonImageDataRaster(Dataset ds, AVList parameters)
        throws ArgumentException, WWRuntimeException
    {
        String pixelFormat = parameters.getStringValue(AVKey.PIXEL_FORMAT);
        if (!AVKey.ELEVATION.equals(pixelFormat))
        ...
        {
            parameters.setValue(AVKey.BYTE_ORDER,
        ...
        ByteBufferRaster   raster = new ByteBufferRaster(width, height, sector, data, parameters);
