...
public class DataConfigurationUtils
{
     ...
     *
     * @param parameters the parameter list to extract a configuration filename from.
     ...
     */
    public static String getDataConfigFilename(AVList parameters, String suffix)
    {
        if (params == null)
        {
        ...
        String path = parameters.getStringValue(AVKey.DATA_CACHE_NAME);
        if (path == null || path.length() == 0)
        {
            return null;
        }

        String filename = parameters.getStringValue(AVKey.DATASET_NAME);

        if (filename == null || filename.length() == 0)
        {
            filename = parameters.getStringValue(AVKey.DISPLAY_NAME);
        }
     ...
     * SharpEarth.avlist.AVKey#DATA_CACHE_NAME}. If the parameter already exists, it's left unchanged.
     * <p/>
     ...
     * @param parameters              the output key-value pairs which receive the DATA_CACHE_NAME parameter. A null
     *                            reference is permitted.
     *
     * @return a reference to parameters, or a new AVList if parameters is null.
     *
     * @throws ArgumentException if the data config file's cache path is null or has length zero.
     */
    public static AVList getDataConfigCacheName(String dataConfigCachePath, AVList parameters)
    {
        ...
        String s = parameters.getStringValue(AVKey.DATA_CACHE_NAME);
        if (s == null || s.length() == 0)
        {
            ...
            if (s != null && s.length() > 0)
            {
                ...
                // for cache paths.
                s = s.replaceAll("\\\\", "/");
                parameters.setValue(AVKey.DATA_CACHE_NAME, s);
            }
        }
     ...
     *
     * @param parameters  the key-value pairs which define the WMS layer configuration parameters.
     ...
     */
    public static Element createWMSLayerConfigElements(AVList parameters, Element context)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        s = parameters.getStringValue(AVKey.WMS_VERSION);
        if (s != null && s.length() > 0)
        {
        ...
        s = parameters.getStringValue(AVKey.GET_MAP_URL);
        if (s != null && s.length() > 0)
        {
     ...
     * @throws ArgumentException if either the parameters or the context are null.
     */
    public static Element createWCSLayerConfigElements(AVList parameters, Element context)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        s = parameters.getStringValue(AVKey.WCS_VERSION);
        if (s != null && s.length() > 0)
        {
            WWXML.setTextAttribute(el, "version", s);
        }
        ...
        s = parameters.getStringValue(AVKey.GET_COVERAGE_URL);
        if (s != null && s.length() > 0)
        {
            Element urlElement = WWXML.getElement(el, "URL", xpath);
            if (urlElement != null)
     ...
     * key-value pairs to parameters. If a parameter from the XML document already exists in parameters, that parameter is
     ...
     * @param parameters     the output key-value pairs which receive the WMS layer parameters. A null reference is
     *                   permitted.
     *
     * @return a reference to parameters, or a new AVList if parameters is null.
     *
     * @throws ArgumentException if the document is null.
     */
    public static AVList getWMSLayerConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
            String message = Logging.getMessage("nullValue.DocumentIsNull");
        ...
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.WMS_VERSION, "Service/@version", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.LAYER_NAMES, "Service/LayerNames", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.STYLE_NAMES, "Service/StyleNames", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.GET_MAP_URL, "Service/GetMapURL", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.GET_CAPABILITIES_URL, "Service/GetCapabilitiesURL",
            xpath);

        parameters.setValue(AVKey.SERVICE, parameters.getValue(AVKey.GET_MAP_URL));
        String serviceURL = parameters.getStringValue(AVKey.SERVICE);
        if (serviceURL != null)
        {
            parameters.setValue(AVKey.SERVICE, WWXML.fixGetMapString(serviceURL));
        }
        ...
        if (layerNames != null)
        {
            parameters.setValue(AVKey.DATASET_NAME, layerNames);
        }

        return parameters;
    }

    public static AVList getWCSConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
            String message = Logging.getMessage("nullValue.DocumentIsNull");
        ...
        // Need to determine these for URLBuilder construction.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.WCS_VERSION, "Service/@version", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.COVERAGE_IDENTIFIERS, "Service/CoverageIdentifiers",
            xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.GET_COVERAGE_URL, "Service/GetCoverageURL", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.GET_CAPABILITIES_URL, "Service/GetCapabilitiesURL",
            xpath);

        parameters.setValue(AVKey.SERVICE, parameters.getValue(AVKey.GET_COVERAGE_URL));
        String serviceURL = parameters.getStringValue(AVKey.SERVICE);
        if (serviceURL != null)
        {
            parameters.setValue(AVKey.SERVICE, WWXML.fixGetMapString(serviceURL));
        ...
        // The dataset name is the layer-names string for WMS elevation models
        String coverages = parameters.getStringValue(AVKey.COVERAGE_IDENTIFIERS);
        if (coverages != null)
        {
            parameters.setValue(AVKey.DATASET_NAME, coverages);
        }

        return parameters;
    }

    public static AVList getWMSLayerConfigParams(WMSCapabilities caps, String[] formatOrderPreference, AVList parameters)
    {
        if (caps == null)
        {
        ...
        String layerNames = parameters.getStringValue(AVKey.LAYER_NAMES);
        String styleNames = parameters.getStringValue(AVKey.STYLE_NAMES);
        ...
        if (WWUtil.isEmpty(coordinateSystem))
        {
            ...
            if (!WWUtil.isEmpty(coordinateSystem))
                parameters.setValue(AVKey.COORDINATE_SYSTEM, coordinateSystem);
        }

        // Define the DISPLAY_NAME and DATASET_NAME from the WMS layer names and styles.
        parameters.setValue(AVKey.DISPLAY_NAME, makeTitle(caps, layerNames, styleNames));
        parameters.setValue(AVKey.DATASET_NAME, layerNames);
        ...
        if (lastUpdate != null)
        {
            parameters.setValue(AVKey.EXPIRY_TIME, lastUpdate);
        }
        ...
        {
            parameters.setValue(AVKey.GET_MAP_URL, mapRequestURIString);
        }
        mapRequestURIString = parameters.getStringValue(AVKey.GET_MAP_URL);
        ...
        {
            parameters.setValue(AVKey.GET_CAPABILITIES_URL, capsRequestURIString);
        }
        ...
        {
            parameters.setValue(AVKey.SERVICE_NAME, OGCConstants.WMS_SERVICE_NAME);
        }
        ...
        {
            parameters.setValue(AVKey.WMS_VERSION, versionString);
        }
        ...
        if (params.getValue(AVKey.DATA_CACHE_NAME) == null)
        {
            try
            {
                ...
                String cacheName = WWIO.formPath(mapRequestURI.getAuthority(), mapRequestURI.getPath(), layerNames,
                    styleNames);
                parameters.setValue(AVKey.DATA_CACHE_NAME, cacheName);
            }
        ...
        if (params.getStringValue(AVKey.IMAGE_FORMAT) == null)
        {
            ...
            parameters.setValue(AVKey.IMAGE_FORMAT, imageFormat);
        }
    ...
    public static AVList getWCSConfigParameters(WCS100Capabilities caps, WCS100DescribeCoverage coverage, AVList parameters)
    {
        if (caps == null)
        {
            String message = Logging.getMessage("nullValue.WMSCapabilities");
        ...
        parameters.setValue(AVKey.SERVICE_NAME, OGCConstants.WCS_SERVICE_NAME);
        parameters.setValue(AVKey.WCS_VERSION, caps.getVersion() != null ? caps.getVersion() : "1.0.0");
        parameters.setValue(AVKey.DISPLAY_NAME, offering.getLabel());
        parameters.setValue(AVKey.COVERAGE_IDENTIFIERS, offering.getName());
        parameters.setValue(AVKey.GET_COVERAGE_URL, caps.getCapability().getGetOperationAddress("GetCoverage"));
        parameters.setValue(AVKey.GET_CAPABILITIES_URL, caps.getCapability().getGetOperationAddress("GetCapabilities"));
        ...
        if (params.getValue(AVKey.DATA_CACHE_NAME) == null)
        {
            try
            {
                ...
                String cacheName = WWIO.formPath(mapRequestURI.getAuthority(), mapRequestURI.getPath(),
                    parameters.getStringValue(AVKey.COVERAGE_IDENTIFIERS));
                parameters.setValue(AVKey.DATA_CACHE_NAME, cacheName);
            }
            catch (URISyntaxException e)
            {
                String message = Logging.getMessage("WCS.RequestMapURLBad",
                    parameters.getStringValue(AVKey.GET_COVERAGE_URL));
                Logging.logger().log(java.util.logging.Level.SEVERE, message, e);
                throw new WWRuntimeException(message);
            }
        }
        ...
        for (String format : offering.getSupportedFormats().getStrings())
        {
            ...
            {
                parameters.setValue(AVKey.IMAGE_FORMAT, format);
                break;
            }
        ...
        if (envelope != null)
        {
            ...
            if (sw != null && ne != null)
            {
                parameters.setValue(AVKey.SECTOR, Sector.fromDegreesAndClamp(sw[1], ne[1], sw[0], ne[0]));
            }
        }
        ...
        if (crs != null)
        {
            parameters.setValue(AVKey.COORDINATE_SYSTEM, crs);
        }
        ...
        if (nullValues != null && nullValues.getSingleValues() != null && nullValues.getSingleValues().size() > 0)
        {
            ...
            if (nullValue != null)
            {
                parameters.setValue(AVKey.MISSING_DATA_SIGNAL, nullValue);
            }
        }
     ...
     *
     * @param parameters parameter list to get the GetCapabilities parameters from.
     *
     * @return a OGC GetCapabilities URL, or null if the necessary parameters are not available.
     *
     * @throws ArgumentException if the parameter list is null.
     */
    public static URL getOGCGetCapabilitiesURL(AVList parameters)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        String uri = parameters.getStringValue(AVKey.GET_CAPABILITIES_URL);
        if (uri == null || uri.length() == 0)
        {
            return null;
        }

        String service = parameters.getStringValue(AVKey.SERVICE_NAME);
     ...
     * returns null.
     *
     * @param parameters parameter list to get the layer names from.
     *
     * @return an array of layer names, or null if none exist.
     *
     * @throws ArgumentException if the parameter list is null.
     */
    public static String[] getOGCLayerNames(AVList parameters)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        String s = parameters.getStringValue(AVKey.LAYER_NAMES);
        if (s == null || s.length() == 0)
        {
            return null;
        }
     ...
     * SharpEarth.avlist.AVKey#DATASET_NAME}</td><td>DatasetName</td><td>String</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#DATA_CACHE_NAME}</td><td>DataCacheName</td><td>String</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#SERVICE}</td><td>Service/URL</td><td>String</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#EXPIRY_TIME}</td><td>ExpiryTime</td><td>Long</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#EXPIRY_TIME}</td><td>LastUpdate</td><td>Long</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#FORMAT_SUFFIX}</td><td>FormatSuffix</td><td>String</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#NUM_LEVELS}</td><td>NumLevels/@count</td><td>Integer</td></tr> <tr><td>{@link
     * SharpEarth.avlist.AVKey#NUM_EMPTY_LEVELS}</td><td>NumLevels/@numEmpty</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#INACTIVE_LEVELS}</td><td>NumLevels/@inactive</td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#SECTOR}</td><td>Sector</td><td>{@link
     * SharpEarth.geom.Sector}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#SECTOR_RESOLUTION_LIMITS}</td><td>SectorResolutionLimit</td>
     * <td>{@link LevelSet.SectorResolution}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TILE_ORIGIN}</td><td>TileOrigin/LatLon</td><td>{@link
     * SharpEarth.geom.LatLon}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TILE_WIDTH}</td><td>TileSize/Dimension/@width</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#TILE_HEIGHT}</td><td>TileSize/Dimension/@height</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#LEVEL_ZERO_TILE_DELTA}</td><td>LastUpdate</td><td>LatLon</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#MAX_ABSENT_TILE_ATTEMPTS}</td><td>MaxAbsentTileAttempts</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#MIN_ABSENT_TILE_CHECK_INTERVAL}</td><td>MinAbsentTileCheckInterval</td><td>Integer</td></tr>
     * </table>
     *
     * @param parameters  the key-value pairs which define the LevelSet configuration parameters.
     ...
     * @throws ArgumentException if either the parameters or the context are null.
     */
    public static Element createLevelSetConfigElements(AVList parameters, Element context)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        // Service properties.
        String s = parameters.getStringValue(AVKey.SERVICE);
        if (s != null && s.length() > 0)
        {
        ...
        if (numLevels != null)
        {
            ...
            s = parameters.getStringValue(AVKey.INACTIVE_LEVELS);
            if (s != null && s.length() > 0)
            {
        ...
        if (params.getValue(AVKey.MAX_ABSENT_TILE_ATTEMPTS) != null ||
            parameters.getValue(AVKey.MIN_ABSENT_TILE_CHECK_INTERVAL) != null)
        {
     ...
     * to parameters. If a parameter from the XML document already exists in parameters, that parameter is ignored. Supported
     ...
     * <tr><td>{@link SharpEarth.avlist.AVKey#TILE_HEIGHT}</td><td>TileSize/Dimension/@height</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#LEVEL_ZERO_TILE_DELTA}</td><td>LastUpdate</td><td>LatLon</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#MAX_ABSENT_TILE_ATTEMPTS}</td><td>AbsentTiles/MaxAttempts</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#MIN_ABSENT_TILE_CHECK_INTERVAL}</td><td>AbsentTiles/MinCheckInterval/Time</td><td>Integer
     * milliseconds</td></tr> </table>
     *
     * @param domElement the XML document root to parse for LevelSet configuration parameters.
     * @param parameters     the output key-value pairs which receive the LevelSet configuration parameters. A null
     *                   reference is permitted.
     *
     * @return a reference to parameters, or a new AVList if parameters is null.
     *
     * @throws ArgumentException if the document is null.
     */
    public static AVList getLevelSetConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
            String message = Logging.getMessage("nullValue.DocumentIsNull");
        ...
        // Title and cache name properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.DATASET_NAME, "DatasetName", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.DATA_CACHE_NAME, "DataCacheName", xpath);

        // Service properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.SERVICE, "Service/URL", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.SERVICE_NAME, "Service/@serviceName", xpath);

        WWXML.checkAndSetLongParam(domElement, parameters, AVKey.EXPIRY_TIME, "ExpiryTime", xpath);
        WWXML.checkAndSetDateTimeParam(domElement, parameters, AVKey.EXPIRY_TIME, "LastUpdate", DATE_TIME_PATTERN, xpath);

        // Image format properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.FORMAT_SUFFIX, "FormatSuffix", xpath);

        // Tile structure properties.
        WWXML.checkAndSetIntegerParam(domElement, parameters, AVKey.NUM_LEVELS, "NumLevels/@count", xpath);
        WWXML.checkAndSetIntegerParam(domElement, parameters, AVKey.NUM_EMPTY_LEVELS, "NumLevels/@numEmpty", xpath);
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.INACTIVE_LEVELS, "NumLevels/@inactive", xpath);
        WWXML.checkAndSetSectorParam(domElement, parameters, AVKey.SECTOR, "Sector", xpath);
        WWXML.checkAndSetSectorResolutionParam(domElement, parameters, AVKey.SECTOR_RESOLUTION_LIMITS,
            "SectorResolutionLimit", xpath);
        WWXML.checkAndSetLatLonParam(domElement, parameters, AVKey.TILE_ORIGIN, "TileOrigin/LatLon", xpath);
        WWXML.checkAndSetIntegerParam(domElement, parameters, AVKey.TILE_WIDTH, "TileSize/Dimension/@width", xpath);
        WWXML.checkAndSetIntegerParam(domElement, parameters, AVKey.TILE_HEIGHT, "TileSize/Dimension/@height", xpath);
        WWXML.checkAndSetLatLonParam(domElement, parameters, AVKey.LEVEL_ZERO_TILE_DELTA, "LevelZeroTileDelta/LatLon",
            xpath);

        // Retrieval properties.
        WWXML.checkAndSetIntegerParam(domElement, parameters, AVKey.MAX_ABSENT_TILE_ATTEMPTS,
            "AbsentTiles/MaxAttempts", xpath);
        WWXML.checkAndSetTimeParamAsInteger(domElement, parameters, AVKey.MIN_ABSENT_TILE_CHECK_INTERVAL,
     ...
     * pairs parameters. If a parameter from the XML document already exists in parameters, that parameter is ignored. Supported
     * key and parameter names are: <table> <th><td>Parameter</td><td>Element Path</td><td>Type</td></th> <tr><td>{@link
     * SharpEarth.avlist.AVKey#DATASET_NAME}</td><td>First Level's dataset</td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#DATA_CACHE_NAME}</td><td>First Level's
     * cacheName</td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#SERVICE}</td><td>First Level's
     * service</td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#EXPIRY_TIME}</td><td>First
     * Level's expiryTime</td><td>Long</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#FORMAT_SUFFIX}</td><td>FirstLevel's
     * formatSuffix</td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#NUM_LEVELS}</td><td>numLevels</td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#NUM_EMPTY_LEVELS}</td><td>1 + index of first non-empty
     * Level</td><td>Integer</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#INACTIVE_LEVELS}</td><td>Comma
     * delimited string of Level numbers</td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#SECTOR}</td><td>sector</td><td>{@link
     * SharpEarth.geom.Sector}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#SECTOR_RESOLUTION_LIMITS}</td><td>sectorLevelLimits</td>
     * <td>{@link LevelSet.SectorResolution}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TILE_ORIGIN}</td><td>tileOrigin</td><td>{@link
     * SharpEarth.geom.LatLon}</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TILE_WIDTH}</td><td>First
     * Level's tileWidth<td><td>Integer</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TILE_HEIGHT}</td><td>First
     * Level's tileHeight</td><td>Integer</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#LEVEL_ZERO_TILE_DELTA}</td><td>levelZeroTileDelta</td><td>LatLon</td></tr>
     * </table>
     *
     * @param levelSet the LevelSet reference to gather configuration parameters from.
     * @param parameters   the output key-value pairs which receive the LevelSet configuration parameters. A null reference
     *                 is permitted.
     *
     * @return a reference to parameters, or a new AVList if parameters is null.
     *
     * @throws ArgumentException if the document is null.
     */
    public static AVList getLevelSetConfigParams(LevelSet levelSet, AVList parameters)
    {
        if (levelSet == null)
        {
        ...
        // Title and cache name properties.
        String s = parameters.getStringValue(AVKey.DATASET_NAME);
        if (s == null || s.length() == 0)
        {
            s = firstLevel.getDataset();
            if (s != null && s.length() > 0)
            {
                parameters.setValue(AVKey.DATASET_NAME, s);
            }
        }

        s = parameters.getStringValue(AVKey.DATA_CACHE_NAME);
        if (s == null || s.length() == 0)
        {
        ...
        // Service properties.
        s = parameters.getStringValue(AVKey.SERVICE);
        if (s == null || s.length() == 0)
        {
            s = firstLevel.getService();
            if (s != null && s.length() > 0)
            {
                parameters.setValue(AVKey.SERVICE, s);
            }
        }

        Object o = parameters.getValue(AVKey.EXPIRY_TIME);
        if (o == null)
        {
            ...
            if (l > 0)
            {
                parameters.setValue(AVKey.EXPIRY_TIME, l);
            }
        }

        // Image format properties.
        s = parameters.getStringValue(AVKey.FORMAT_SUFFIX);
        if (s == null || s.length() == 0)
        {
            s = firstLevel.getFormatSuffix();
            if (s != null && s.length() > 0)
            {
                parameters.setValue(AVKey.FORMAT_SUFFIX, s);
            }
        }
        ...
        if (o == null)
        {
            parameters.setValue(AVKey.NUM_LEVELS, levelSet.getNumLevels());
        }

        o = parameters.getValue(AVKey.NUM_EMPTY_LEVELS);
        if (o == null)
        {
            parameters.setValue(AVKey.NUM_EMPTY_LEVELS, getNumEmptyLevels(levelSet));
        }
        ...
        if (s == null || s.length() == 0)
        {
            ...
            if (s != null && s.length() > 0)
            {
                parameters.setValue(AVKey.INACTIVE_LEVELS, s);
            }
        }
        ...
        if (o == null)
        {
            ...
            {
                parameters.setValue(AVKey.SECTOR_RESOLUTION_LIMITS, srs);
            }
        }
        ...
        if (o == null)
        {
            ...
            if (ll != null)
            {
                parameters.setValue(AVKey.TILE_ORIGIN, ll);
            }
        }
        ...
        if (o == null)
        {
            parameters.setValue(AVKey.TILE_WIDTH, firstLevel.getTileWidth());
        }

        o = parameters.getValue(AVKey.TILE_HEIGHT);
        if (o == null)
        {
            parameters.setValue(AVKey.TILE_HEIGHT, firstLevel.getTileHeight());
        }

        o = parameters.getValue(AVKey.LEVEL_ZERO_TILE_DELTA);
        if (o == null)
        {
            LatLon ll = levelSet.getLevelZeroTileDelta();
            if (ll != null)
            {
                parameters.setValue(AVKey.LEVEL_ZERO_TILE_DELTA, ll);
            }
        }
     ...
     * key-value pairs to parameters. If a parameter from the LayerSet document already exists in parameters, that parameter is
     * ignored. Supported key and parameter names are: <table> <tr><th>Parameter</th><th>Element
     * Path</th><th>Type</th></tr> <tr><td>{@link SharpEarth.avlist.AVKey#DISPLAY_NAME}</td><td>QuadTileSet/Name<td></td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#DATASET_NAME}</td><td>QuadTileSet/Name<td></td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#OPACITY}</td><td>QuadTileSet/Opacity<td></td><td>Double</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#SERVICE_NAME}</td><td>"Offline" (string
     * constant)<td></td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#FORMAT_SUFFIX}</td><td>QuadTileSet/ImageAccessor/ImageFileExtension<td></td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#IMAGE_FORMAT}</td><td>QuadTileSet/ImageAccessor/ImageFileExtension
     * (converted to mime type)<td></td><td>String</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#AVAILABLE_IMAGE_FORMATS}</td><td>QuadTileSet/ImageAccessor/ImageFileExtension
     * (converted to mime type)<td></td><td>String array</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#NUM_LEVELS}</td><td>QuadTileSet/ImageAccessor/NumberLevels<td></td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#NUM_EMPTY_LEVELS}</td><td>0 (integer
     * constant)<td></td><td>Integer</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#SECTOR}</td><td>QuadTileSet/BoundingBox<td></td><td>Sector</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#TILE_ORIGIN}</td><td>(-90, -180) (geographic location
     * constant)<td></td><td>LatLon</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#LEVEL_ZERO_TILE_DELTA}</td><td>QuadTileSet/ImageAccessor/LevelZeroTileSizeDegrees<td></td><td>LatLon</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#TILE_WIDTH}</td><td>QuadTileSet/ImageAccessor/TextureSizePixels<td></td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#TILE_HEIGHT}</td><td>QuadTileSet/ImageAccessor/TextureSizePixels<td></td><td>Integer</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#NETWORK_RETRIEVAL_ENABLED}</td><td>false (boolean
     * constant)<td></td><td>Boolean</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#TEXTURE_FORMAT}</td><td>"image/dds"<td></td><td>String</td></tr>
     * <tr><td>{@link SharpEarth.avlist.AVKey#USE_MIP_MAPS}</td><td>true (boolean
     * constant)<td></td><td>Boolean</td></tr> <tr><td>{@link SharpEarth.avlist.AVKey#USE_TRANSPARENT_TEXTURES}</td><td>true
     ...
     * @param parameters     the output key-value pairs which receive the LayerSet configuration parameters. A null
     *                   reference is permitted.
     *
     * @return a reference to parameters, or a new AVList if parameters is null.
     *
     * @throws ArgumentException if the document is null.
     */
    public static AVList getWWDotNetLayerSetConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
            String message = Logging.getMessage("nullValue.DocumentIsNull");
        ...
        // Title and cache name properties.
        WWXML.checkAndSetStringParam(el, parameters, AVKey.DISPLAY_NAME, "Name", xpath);
        WWXML.checkAndSetStringParam(el, parameters, AVKey.DATASET_NAME, "Name", xpath);
        ...
        if (params.getValue(AVKey.OPACITY) == null)
        {
            ...
            if (d != null)
            {
                parameters.setValue(AVKey.OPACITY, d / 255d);
            }
        }
        ...
        if (params.getValue(AVKey.SERVICE_NAME) == null)
        {
            parameters.setValue(AVKey.SERVICE_NAME, "Offline");
        }
        ...
        if (params.getValue(AVKey.FORMAT_SUFFIX) != null)
        {
            ...
            if (s != null)
            {
                ...
                {
                    parameters.setValue(AVKey.AVAILABLE_IMAGE_FORMATS, new String[] {s});
                }
            }
        }
        ...
        {
            parameters.setValue(AVKey.TEXTURE_FORMAT, DEFAULT_TEXTURE_FORMAT);
        }

        // Tile structure properties.
        WWXML.checkAndSetIntegerParam(el, parameters, AVKey.NUM_LEVELS, "ImageAccessor/NumberLevels", xpath);
        ...
        if (params.getValue(AVKey.SECTOR) == null)
        {
            ...
            if (s != null)
            {
                parameters.setValue(AVKey.SECTOR, s);
            }
        }

        if (params.getValue(AVKey.TILE_ORIGIN) == null)
        {
            parameters.setValue(AVKey.TILE_ORIGIN, new LatLon(Angle.NEG90, Angle.NEG180));
        }
        ...
        if (tileDimension != null)
        {
            ...
            {
                parameters.setValue(AVKey.TILE_WIDTH, tileDimension);
            }
            if (params.getValue(AVKey.TILE_HEIGHT) == null)
            {
                parameters.setValue(AVKey.TILE_HEIGHT, tileDimension);
            }
        }
        ...
        {
            parameters.setValue(AVKey.NETWORK_RETRIEVAL_ENABLED, false);
        }
        if (params.getValue(AVKey.USE_MIP_MAPS) == null)
        {
            parameters.setValue(AVKey.USE_MIP_MAPS, true);
        }
