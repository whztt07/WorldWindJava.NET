...
namespace SharpEarth.globes
{
  ...
  public interface ElevationModel : WWObject, Restorable, Disposable
  {
     ...
     *                         elevation model for {@link SharpEarth.terrain.CompoundElevationModel}. The
     *                         entries must be in the same order as the elevations in {@link
     *                         SharpEarth.terrain.CompoundElevationModel}.
     * @param buffer           an array in which to place the returned elevations. The array must be pre-allocated and
     *                         contain at least as many elements as the list of locations.
     *
     ...
     *         {@link SharpEarth.terrain.CompoundElevationModel}.
     *
     * @throws ArgumentException if either the sector, latlons list, target resolutions array or elevations array
     *                                  is null.
     * @see #setMissingDataSignal(double)
     */
    double[] getElevations<T>( Sector sector, List<T> latlons, double[] targetResolution, double[] buffer ) where T : LatLon;
     ...
     *         {@link SharpEarth.terrain.CompoundElevationModel}.
     *
     * @throws ArgumentException if either the sector, latlons list, target resolutions array or elevations
     *                                  array
     * @see #setMissingDataSignal(double)
     */
    double[] getUnmappedElevations<T>( Sector sector, List<T> latlons, double[] targetResolution, double[] buffer ) where T : LatLon;
     ...
     * @throws ArgumentException if either the sector, list of locations or buffer is null, if the buffer size is
