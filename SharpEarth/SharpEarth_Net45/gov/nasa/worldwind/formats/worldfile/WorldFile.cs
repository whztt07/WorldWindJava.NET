...
public class WorldFile
{
     ...
     *
     * @throws ArgumentException if the data file is null.
     ...
     *
     * @throws ArgumentException if the values array is null or has a length less than six, or the image width
     ...
     *
     * @throws ArgumentException if the values array is null or has a length less than six, , the image width and
     ...
     * follows: <ul> <li>Geographic - {@link SharpEarth.avlist.AVKey#COORDINATE_SYSTEM} set to {@link
     * SharpEarth.avlist.AVKey#COORDINATE_SYSTEM_GEOGRAPHIC}.</li> <li>Projected coordinate system: Universal
     * Transverse Mercator (UTM) - {@link SharpEarth.avlist.AVKey#COORDINATE_SYSTEM} set to {@link
     * SharpEarth.avlist.AVKey#COORDINATE_SYSTEM_PROJECTED} and {@link SharpEarth.avlist.AVKey#PROJECTION_NAME}
     * set to {@link SharpEarth.avlist.AVKey#PROJECTION_UTM}. {@link SharpEarth.avlist.AVKey#PROJECTION_HEMISPHERE}
     * set to either {@link SharpEarth.avlist.AVKey#NORTH} or {@link SharpEarth.avlist.AVKey#SOUTH}.
     * {@link SharpEarth.avlist.AVKey#PROJECTION_ZONE} set to an integer in the range 1-60</li> <li>Projected
     * coordinate system: unknown projection - {@link SharpEarth.avlist.AVKey#COORDINATE_SYSTEM} set to {@link
     * SharpEarth.avlist.AVKey#COORDINATE_SYSTEM_PROJECTED} and {@link SharpEarth.avlist.AVKey#PROJECTION_NAME}
     * set to {@link SharpEarth.avlist.AVKey#PROJECTION_UNKNOWN}.</li> <li>Unknown coordinate system - {@link
     * SharpEarth.avlist.AVKey#COORDINATE_SYSTEM} set to {@link SharpEarth.avlist.AVKey#COORDINATE_SYSTEM_UNKNOWN}.
     ...
     * @param parameters the coordinate system parameter list, or null to indicate a parameter list should be created.
     *
     * @return the coordinate system parameter list.
     *
     * @throws ArgumentException if <code>text</code> is null.
     */
    public static AVList decodeOGCCoordinateSystemWKT(String text, AVList parameters)
    {
        if (text == null)
        {
        ...
        try
        {
            ...
            else if ((csMatcher = PROJCS_WKT_PATTERN.matcher(text)).matches())
            {
                ...
                {
                    parameters.setValue(AVKey.PROJECTION_NAME, AVKey.PROJECTION_UTM);
                    ...
                    if (s != null)
                    {
                        ...
                        if (i != null && i >= 1 && i <= 60)
                            parameters.setValue(AVKey.PROJECTION_ZONE, i);
                    }
                    ...
                    if (s != null)
                    {
                        ...
                        if (s.startsWith("N") || s.startsWith("n"))
                            parameters.setValue(AVKey.PROJECTION_HEMISPHERE, AVKey.NORTH);
                        else if (s.startsWith("S") || s.startsWith("s"))
                            parameters.setValue(AVKey.PROJECTION_HEMISPHERE, AVKey.SOUTH);
                    }
                ...
                {
                    parameters.setValue(AVKey.PROJECTION_NAME, AVKey.PROJECTION_UNKNOWN);
                }
            }
            else
            {
     ...
     *
     * @param source the source to local file path.
     * @param parameters AVList for storing parameters
     *
     * @throws IOException if the world file could not be read
     */
    public static void readWorldFiles(Object source, AVList parameters) throws java.io.IOException
    {
        File file = WWIO.getFileForLocalAddress(source);
        if (null == file)
        {
        ...
        if (o != null && o is int[])
        {
            ...
            if (!params.hasKey(AVKey.WIDTH))
                parameters.setValue(AVKey.WIDTH, size[0]);

            if (!params.hasKey(AVKey.HEIGHT))
                parameters.setValue(AVKey.HEIGHT, size[1]);
        }
    }
}
}
