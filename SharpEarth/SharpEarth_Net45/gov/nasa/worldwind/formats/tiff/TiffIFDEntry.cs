...
public class TiffIFDEntry implements Comparable<TiffIFDEntry> {
    ...
    public TiffIFDEntry(int tag, int type, long value) throws ArgumentException {
        this(tag, type, 1, value, null);
    }

    public TiffIFDEntry(int tag, int type, long count, long valOffset, ByteBuffer data) throws ArgumentException {
        this.tag = tag;
        this.type = type;
        this.count = count;
