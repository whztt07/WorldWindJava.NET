...
public class BasicTiledImageLayer extends TiledImageLayer implements BulkRetrievable
{
    ...
    public BasicTiledImageLayer(AVList parameters)
    {
        ...
        String s = parameters.getStringValue(AVKey.DISPLAY_NAME);
        if (s != null)
            this.setName(s);

        String[] strings = (String[]) parameters.getValue(AVKey.AVAILABLE_IMAGE_FORMATS);
        if (strings != null && strings.length > 0)
            this.setAvailableImageFormats(strings);

        s = parameters.getStringValue(AVKey.TEXTURE_FORMAT);
        if (s != null)
            this.setTextureFormat(s);

        Double d = (Double) parameters.getValue(AVKey.OPACITY);
        if (d != null)
            this.setOpacity(d);

        d = (Double) parameters.getValue(AVKey.MAX_ACTIVE_ALTITUDE);
        if (d != null)
            this.setMaxActiveAltitude(d);

        d = (Double) parameters.getValue(AVKey.MIN_ACTIVE_ALTITUDE);
        if (d != null)
            this.setMinActiveAltitude(d);

        d = (Double) parameters.getValue(AVKey.MAP_SCALE);
        if (d != null)
            this.setValue(AVKey.MAP_SCALE, d);

        d = (Double) parameters.getValue(AVKey.DETAIL_HINT);
        if (d != null)
            this.setDetailHint(d);

        Boolean b = (Boolean) parameters.getValue(AVKey.FORCE_LEVEL_ZERO_LOADS);
        if (b != null)
            this.setForceLevelZeroLoads(b);

        b = (Boolean) parameters.getValue(AVKey.RETAIN_LEVEL_ZERO_TILES);
        if (b != null)
            this.setRetainLevelZeroTiles(b);

        b = (Boolean) parameters.getValue(AVKey.NETWORK_RETRIEVAL_ENABLED);
        if (b != null)
            this.setNetworkRetrievalEnabled(b);

        b = (Boolean) parameters.getValue(AVKey.USE_MIP_MAPS);
        if (b != null)
            this.setUseMipMaps(b);

        b = (Boolean) parameters.getValue(AVKey.USE_TRANSPARENT_TEXTURES);
        if (b != null)
            this.setUseTransparentTextures(b);

        Object o = parameters.getValue(AVKey.URL_CONNECT_TIMEOUT);
        if (o != null)
            this.setValue(AVKey.URL_CONNECT_TIMEOUT, o);

        o = parameters.getValue(AVKey.URL_READ_TIMEOUT);
        if (o != null)
            this.setValue(AVKey.URL_READ_TIMEOUT, o);

        o = parameters.getValue(AVKey.RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT);
        if (o != null)
            this.setValue(AVKey.RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT, o);

        ScreenCredit sc = (ScreenCredit) parameters.getValue(AVKey.SCREEN_CREDIT);
        if (sc != null)
            this.setScreenCredit(sc);

        if (params.getValue(AVKey.TRANSPARENCY_COLORS) != null)
            this.setValue(AVKey.TRANSPARENCY_COLORS, parameters.getValue(AVKey.TRANSPARENCY_COLORS));
    ...
    public BasicTiledImageLayer(Document dom, AVList parameters)
    {
        this(dom.getDocumentElement(), parameters);
    }

    public BasicTiledImageLayer(Element domElement, AVList parameters)
    {
        this(getParamsFromDocument(domElement, parameters));
    }
    ...
    protected static AVList getParamsFromDocument(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
        ...
        getTiledImageLayerConfigParams(domElement, parameters);
        setFallbacks(params);
    ...
    protected static void setFallbacks(AVList parameters)
    {
        ...
        if (params.getValue(AVKey.TILE_WIDTH) == null)
            parameters.setValue(AVKey.TILE_WIDTH, 512);

        if (params.getValue(AVKey.TILE_HEIGHT) == null)
            parameters.setValue(AVKey.TILE_HEIGHT, 512);

        if (params.getValue(AVKey.FORMAT_SUFFIX) == null)
            parameters.setValue(AVKey.FORMAT_SUFFIX, ".dds");

        if (params.getValue(AVKey.NUM_LEVELS) == null)
            parameters.setValue(AVKey.NUM_LEVELS, 19); // approximately 0.1 meters per pixel
    ...
    protected static class RequestTask implements Runnable, Comparable<RequestTask>
    {
         ...
         *
         * @throws ArgumentException if <code>that</code> is null
         */
     ...
     * {@link SharpEarth.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL} indicates the retrieval succeeded, {@link
     * SharpEarth.avlist.AVKey#RETRIEVAL_STATE_ERROR} indicates the retrieval failed with errors, and
     ...
     *         SharpEarth.avlist.AVKey#RETRIEVAL_STATE_ERROR} if the retrieval failed with errors, and
     ...
     * @param parameters the parameter list describing the WMS layer names associated with this Layer.
     *
    ...
    protected void writeConfigurationParams(FileStore fileStore, AVList parameters)
    {
