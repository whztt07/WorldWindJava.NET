 ...
 * All Rights Reserved.
 */
using java.util.concurrent.PriorityBlockingQueue;
using java.util;
using java.net;
using java.io;
...
//                }
    ...
    /**
     * Creates a configuration document for a TiledImageLayer described by the specified parameters. The returned document
     * may be used as a construction parameter to {@link SharpEarth.layers.BasicTiledImageLayer}.
     *
     * @param parameters parameters describing the TiledImageLayer.
     *
     * @return a configuration document for the TiledImageLayer.
     */
    public static Document createTiledImageLayerConfigDocument(AVList parameters)
    {
     ...
     * common layer and LevelSet configuration parameters by invoking {@link SharpEarth.layers.AbstractLayer#createLayerConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)} and {@link DataConfigurationUtils#createLevelSetConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)}.
     *
     * @param parameters  the key-value pairs which define the TiledImageLayer configuration parameters.
     ...
     */
    public static Element createTiledImageLayerConfigElements(AVList parameters, Element context)
    {
        if (params == null)
        {
        ...
        if (params.getValue(AVKey.URL_CONNECT_TIMEOUT) != null ||
            parameters.getValue(AVKey.URL_READ_TIMEOUT) != null ||
            parameters.getValue(AVKey.RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT) != null)
        {
     ...
     * common layer and LevelSet configuration parameters by invoking {@link SharpEarth.layers.AbstractLayer#getLayerConfigParams(org.w3c.dom.Element,
     ...
     * @param parameters     the output key-value pairs which recieve the TiledImageLayer configuration parameters. A null
     ...
     */
    public static AVList getTiledImageLayerConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
        ...
        // Common layer properties.
        AbstractLayer.getLayerConfigParams(domElement, parameters);
        ...
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.TEXTURE_FORMAT, "TextureFormat", xpath);
        ...
        // Optional behavior properties.
        WWXML.checkAndSetBooleanParam(domElement, parameters, AVKey.FORCE_LEVEL_ZERO_LOADS, "ForceLevelZeroLoads", xpath);
        WWXML.checkAndSetBooleanParam(domElement, parameters, AVKey.RETAIN_LEVEL_ZERO_TILES, "RetainLevelZeroTiles", xpath);
        WWXML.checkAndSetBooleanParam(domElement, parameters, AVKey.USE_MIP_MAPS, "UseMipMaps", xpath);
        WWXML.checkAndSetBooleanParam(domElement, parameters, AVKey.USE_TRANSPARENT_TEXTURES, "UseTransparentTextures",
            xpath);
        WWXML.checkAndSetDoubleParam(domElement, parameters, AVKey.DETAIL_HINT, "DetailHint", xpath);
        WWXML.checkAndSetColorArrayParam(domElement, parameters, AVKey.TRANSPARENCY_COLORS, "TransparencyColors/Color",
            xpath);

        // Retrieval properties. Convert the Long time values to Integers, because BasicTiledImageLayer is expecting
        // Integer values.
        WWXML.checkAndSetTimeParamAsInteger(domElement, parameters, AVKey.URL_CONNECT_TIMEOUT,
            "RetrievalTimeouts/ConnectTimeout/Time", xpath);
        WWXML.checkAndSetTimeParamAsInteger(domElement, parameters, AVKey.URL_READ_TIMEOUT,
            "RetrievalTimeouts/ReadTimeout/Time", xpath);
        WWXML.checkAndSetTimeParamAsInteger(domElement, parameters, AVKey.RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT,
        ...
        getLegacyTiledImageLayerConfigParams(domElement, parameters);
     ...
     * output as key-value pairs to parameters. If a parameter from the XML document already exists in parameters, that
     ...
     * @throws ArgumentException if the document is null.
     */
    protected static AVList getLegacyTiledImageLayerConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
            String message = Logging.getMessage("nullValue.DocumentIsNull");
        ...
        Object o = parameters.getValue(AVKey.TEXTURE_FORMAT);
        if (o == null)
        {
            Boolean b = WWXML.getBoolean(domElement, "CompressTextures", xpath);
            if (b != null && b)
                parameters.setValue(AVKey.TEXTURE_FORMAT, "image/dds");
        }
     ...
     * @see ImageUtil#mergeImage(gov.nasa.worldwind.geom.Sector, SharpEarth.geom.Sector, double,
