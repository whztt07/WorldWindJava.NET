...
public class BasicElevationModel extends AbstractElevationModel implements BulkRetrievable
{
    ...
    public BasicElevationModel(AVList parameters)
    {
        if (params == null)
        {
        ...
        String s = parameters.getStringValue(AVKey.BYTE_ORDER);
        if (s != null)
            this.setByteOrder(s);

        Double d = (Double) parameters.getValue(AVKey.DETAIL_HINT);
        if (d != null)
            this.setDetailHint(d);

        s = parameters.getStringValue(AVKey.DISPLAY_NAME);
        if (s != null)
            this.setName(s);

        d = (Double) parameters.getValue(AVKey.ELEVATION_MIN);
        this.minElevation = d != null ? d : 0;

        d = (Double) parameters.getValue(AVKey.ELEVATION_MAX);
        this.maxElevation = d != null ? d : 0;

        Long lo = (Long) parameters.getValue(AVKey.EXPIRY_TIME);
        if (lo != null)
            parameters.setValue(AVKey.EXPIRY_TIME, lo);

        d = (Double) parameters.getValue(AVKey.MISSING_DATA_SIGNAL);
        if (d != null)
            this.setMissingDataSignal(d);

        d = (Double) parameters.getValue(AVKey.MISSING_DATA_REPLACEMENT);
        if (d != null)
            this.setMissingDataReplacement(d);

        Boolean b = (Boolean) parameters.getValue(AVKey.NETWORK_RETRIEVAL_ENABLED);
        if (b != null)
            this.setNetworkRetrievalEnabled(b);

        s = parameters.getStringValue(AVKey.DATA_TYPE);
        if (s != null)
            this.setElevationDataType(s);

        s = parameters.getStringValue(AVKey.ELEVATION_EXTREMES_FILE);
        if (s != null)
            this.loadExtremeElevations(s);

        b = (Boolean) parameters.getValue(AVKey.DELETE_CACHE_ON_EXIT);
        if (b != null)
        ...
        this.setValue(AVKey.CONSTRUCTION_PARAMETERS, parameters.copy());
    ...
    public BasicElevationModel(Document dom, AVList parameters)
    {
        this(dom.getDocumentElement(), parameters);
    }

    public BasicElevationModel(Element domElement, AVList parameters)
    {
        this(getBasicElevationModelConfigParams(domElement, parameters));
    }
    ...
    protected static void setFallbacks(AVList parameters)
    {
        ...
        if (params.getValue(AVKey.NUM_LEVELS) == null)
            parameters.setValue(AVKey.NUM_LEVELS, 2);
    ...
    public double getBestResolution(Sector sector)
    {
        ...
        return level != null ? level.getTexelSize() : Double.MaxValue;
    }
...
//**************************************************************//
    ...
    protected BufferWrapper makeTiffElevations(URL url) throws IOException, URISyntaxException
    {
        ...
        // Setup parameters to instruct BufferWrapper on how to interpret the ByteBuffer.
        AVList bufferParams = new AVListImpl();
        bufferParams.setValues(raster.copy()); // copies parameters from avlist
     ...
     *
     * @throws ArgumentException if the sector is null or the resolution is less than  zero.
...
//        }
    ...
    protected Elevations getElevations(Sector requestedSector, LevelSet levelSet, int targetLevelNumber)
    {
        ...
        {
            // Double.MaxValue is a signal for no in-memory tile for a given region of the sector.
            elevations = new Elevations(this, Double.MaxValue);
...
//    }
     ...
     * retrieval state: {@link SharpEarth.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL} indicates the retrieval
     * succeeded, {@link SharpEarth.avlist.AVKey#RETRIEVAL_STATE_ERROR} indicates the retrieval failed with
     ...
     *
     * @return {@link SharpEarth.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL} if the retrieval succeeded, {@link
     * SharpEarth.avlist.AVKey#RETRIEVAL_STATE_ERROR} if the retrieval failed with errors, and <code>null</code>
    ...
    protected String retrieveResources()
    {
        ...
        this.initFromOGCCapabilitiesResource(caps, parameters);
     ...
     * @param parameters the parameter list describing the WMS layer names associated with this ElevationModel.
     *
     * @throws ArgumentException if either the Capabilities or the parameter list is null.
     */
    protected void initFromOGCCapabilitiesResource(WMSCapabilities caps, AVList parameters)
    {
        if (caps == null)
        {
    ...
    protected bool isRetrieveResources()
    {
        ...
        Boolean b = (Boolean) parameters.getValue(AVKey.RETRIEVE_PROPERTIES_FROM_SERVICE);
        return b != null && b;
    }
    ...
    /**
     * Creates a configuration document for a BasicElevationModel described by the specified parameters. The returned
     * document may be used as a construction parameter to {@link SharpEarth.terrain.BasicElevationModel}.
     *
     * @param parameters parameters describing a BasicElevationModel.
     *
     * @return a configuration document for the BasicElevationModel.
     */
    public static Document createBasicElevationModelConfigDocument(AVList parameters)
    {
     ...
     * elevation model and LevelSet configuration parameters by invoking {@link SharpEarth.terrain.AbstractElevationModel#createElevationModelConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)} and {@link DataConfigurationUtils#createLevelSetConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)}.
     *
     * @param parameters  the key-value pairs which define the BasicElevationModel configuration parameters.
     ...
     */
    public static Element createBasicElevationModelConfigElements(AVList parameters, Element context)
    {
        if (params == null)
        {
            String message = Logging.getMessage("nullValue.ParametersIsNull");
        ...
        if (params.getValue(AVKey.DATA_TYPE) != null || parameters.getValue(AVKey.BYTE_ORDER) != null)
        {
            ...
            s = parameters.getStringValue(AVKey.BYTE_ORDER);
            if (s != null && s.length() > 0)
            {
     ...
     * elevation model and LevelSet configuration parameters by invoking {@link SharpEarth.terrain.AbstractElevationModel#getElevationModelConfigParams(org.w3c.dom.Element,
     * SharpEarth.avlist.AVList)} and {@link SharpEarth.util.DataConfigurationUtils#getLevelSetConfigParams(org.w3c.dom.Element,
     * SharpEarth.avlist.AVList)}.
     *
     * @param domElement the XML document root to parse for BasicElevationModel configuration parameters.
     * @param parameters     the output key-value pairs which recieve the BasicElevationModel configuration parameters. A
     ...
     */
    public static AVList getBasicElevationModelConfigParams(Element domElement, AVList parameters)
    {
        if (domElement == null)
        {
        ...
        // Common elevation model properties.
        AbstractElevationModel.getElevationModelConfigParams(domElement, parameters);

        // LevelSet properties.
        DataConfigurationUtils.getLevelSetConfigParams(domElement, parameters);

        // Service properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.SERVICE_NAME, "Service/@serviceName", xpath);
        WWXML.checkAndSetBooleanParam(domElement, parameters, AVKey.RETRIEVE_PROPERTIES_FROM_SERVICE,
            "RetrievePropertiesFromService", xpath);

        // Image format properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.IMAGE_FORMAT, "ImageFormat", xpath);
        WWXML.checkAndSetUniqueStringsParam(domElement, parameters, AVKey.AVAILABLE_IMAGE_FORMATS,
        ...
        if (params.getValue(AVKey.BYTE_ORDER) == null)
        {
            ...
            if (s != null && s.length() > 0)
            {
                ...
                if (s != null && s.length() > 0)
                    parameters.setValue(AVKey.BYTE_ORDER, s);
            }
        }

        // Elevation data properties.
        WWXML.checkAndSetStringParam(domElement, parameters, AVKey.ELEVATION_EXTREMES_FILE, "ExtremeElevations/FileName",
            xpath);
        WWXML.checkAndSetDoubleParam(domElement, parameters, AVKey.ELEVATION_MAX, "ExtremeElevations/@max", xpath);
        WWXML.checkAndSetDoubleParam(domElement, parameters, AVKey.ELEVATION_MIN, "ExtremeElevations/@min", xpath);
    ...
    protected void writeConfigurationParams(AVList parameters, FileStore fileStore)
    {
        ...
        if (!this.needsConfigurationFile(fileStore, fileName, parameters, false))
            return;
        ...
        synchronized (this.fileLock)
        {
            ...
            if (!this.needsConfigurationFile(fileStore, fileName, parameters, true))
                return;

            this.doWriteConfigurationParams(fileStore, fileName, parameters);
        }
    }

    protected void doWriteConfigurationParams(FileStore fileStore, String fileName, AVList parameters)
    {
    ...
    protected AVList getConfigurationParams(AVList parameters)
    {
        if (params == null)
            parameters = new AVListImpl();
        ...
        if (constructionParams != null)
            parameters.setValues(constructionParams);

        // Gather any missing LevelSet parameters from the LevelSet itself.
        DataConfigurationUtils.getLevelSetConfigParams(this.getLevels(), parameters);
        ...
        if (params.getValue(AVKey.BYTE_ORDER) == null)
            parameters.setValue(AVKey.BYTE_ORDER, this.getElevationDataByteOrder());

        if (params.getValue(AVKey.DATA_TYPE) == null)
            parameters.setValue(AVKey.DATA_TYPE, this.getElevationDataType());

        if (params.getValue(AVKey.MISSING_DATA_SIGNAL) == null)
            parameters.setValue(AVKey.MISSING_DATA_SIGNAL, this.getMissingDataSignal());
        ...
        AVList parameters)
    {
        ...
        if (i != null)
            parameters.setValue(AVKey.NUM_EMPTY_LEVELS, i);

        i = rs.getStateValueAsInteger(context, AVKey.NUM_LEVELS);
        if (i != null)
            parameters.setValue(AVKey.NUM_LEVELS, i);

        i = rs.getStateValueAsInteger(context, AVKey.TILE_WIDTH);
        if (i != null)
            parameters.setValue(AVKey.TILE_WIDTH, i);

        i = rs.getStateValueAsInteger(context, AVKey.TILE_HEIGHT);
        if (i != null)
            parameters.setValue(AVKey.TILE_HEIGHT, i);
        ...
        if (d != null)
        {
            parameters.setValue(AVKey.ELEVATION_MIN, d);
        }
        else
        {
            if (sb.length() > 0)
        ...
        if (d != null)
        {
            parameters.setValue(AVKey.ELEVATION_MAX, d);
        }
        else
        {
            if (sb.length() > 0)
